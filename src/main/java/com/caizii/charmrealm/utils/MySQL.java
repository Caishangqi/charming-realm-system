...
public class MySQL {
    public static String type = CharmRealm.JavaPlugin.getConfig().getString("Type");

    public static String host = CharmRealm.JavaPlugin.getConfig().getString("Host");

    public static String port = String.valueOf(CharmRealm.JavaPlugin.getConfig().getInt("Port"));

    public static String database = CharmRealm.JavaPlugin.getConfig().getString("Database");

    public static String username = CharmRealm.JavaPlugin.getConfig().getString("Username");

    public static String password = CharmRealm.JavaPlugin.getConfig().getString("Password");
    ...
    public static void autoUpdateServer() {
        (new BukkitRunnable() {
            public void run() {
                ...
                try {
                    ...
                    ps.setString(1, CharmRealm.JavaPlugin.getConfig().getString("Server"));
                    res = ps.executeQuery();
                    res.last();
                    ...
                    if (amount == 0) {
                        ...
                        ps.setString(2, CharmRealm.JavaPlugin.getConfig().getString("Server"));
                        ps.executeUpdate();
                    }
        ...
        }).runTaskTimerAsynchronously((Plugin) CharmRealm.JavaPlugin, 0L, 60L);
    }
    ...
    public static boolean CheckIsAHome(String name) {
        ...
        try {
            ps = con.prepareStatement("SELECT * FROM SelfHomeMain_Users WHERE Name = ?");
            ps.setString(1, name.replace(CharmRealm.pluginVariable.world_prefix, ""));
            res = ps.executeQuery();
            if (res.next())
                check = true;
        } catch (SQLException e) {
    ...
    public static void data_import(CommandSender p) {
        ...
        File folder = new File(
                String.valueOf(String.valueOf(CharmRealm.JavaPlugin.getDataFolder().getPath().toString())) + CharmRealm.pluginVariable.file_loc_prefix + "playerdata");
        ...
        for (int c = 0; c < list_files.length; c++) {
            ...
            String want_to = temp.getPath().replace(
                            String.valueOf(String.valueOf(CharmRealm.JavaPlugin.getDataFolder().getPath().toString())) + CharmRealm.pluginVariable.file_loc_prefix + "playerdata", "")
                    .replace(".yml", "").replace(CharmRealm.pluginVariable.file_loc_prefix, "");
            ...
            try {
                ...
                res = ps.executeQuery();
                if (res.next()) {
                    String str = CharmRealm.pluginVariable.Lang_YML.getString("ImportButHasAlreadyExist");
            ...
            amount++;
            String str1 = CharmRealm.pluginVariable.Lang_YML.getString("ImportSuccess");
            if (str1.contains("<Name>"))
                str1 = str1.replace("<Name>", want_to);
            p.sendMessage(str1);
            continue;
        }
        String msg = CharmRealm.pluginVariable.Lang_YML.getString("ImportFinal");
    ...
    public static void data_export(CommandSender p) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (!Server.equalsIgnoreCase(CharmRealm.JavaPlugin.getConfig().getString("Server"))) {
                    String str = CharmRealm.pluginVariable.Lang_YML.getString("ExportButServerNotEqual");
                    if (str.contains("<Name>"))
                ...
                File playerdata = new File(String.valueOf(String.valueOf(CharmRealm.JavaPlugin.getDataFolder().getPath().toString())) +
                        CharmRealm.pluginVariable.file_loc_prefix + "playerdata" + CharmRealm.pluginVariable.file_loc_prefix + name + ".yml");
                if (playerdata.exists()) {
                    String str = CharmRealm.pluginVariable.Lang_YML.getString("ExportButHasAlreadyExist");
                    if (str.contains("<Name>"))
                        str = str.replace("<Name>", name);
                    p.sendMessage(str);
                    continue;
                ...
                String str1 = CharmRealm.pluginVariable.Lang_YML.getString("ExportSuccess");
                if (str1.contains("<Name>"))
                    str1 = str1.replace("<Name>", name);
                p.sendMessage(str1);
                amount++;
            }
            String temp = CharmRealm.pluginVariable.Lang_YML.getString("ExportFinal");
    ...
    public static void init() {
        ...
        try {
            ...
            if (ss != 0)
                Bukkit.getConsoleSender().sendMessage(CharmRealm.pluginVariable.Lang_YML.getString("CreateDataBaseTableSuccess"));
            ...
            Bukkit.getConsoleSender().sendMessage(CharmRealm.pluginVariable.Lang_YML.getString("CreateDataBaseTableError"));
            e.printStackTrace();
            return;
        } finally {
            try {
