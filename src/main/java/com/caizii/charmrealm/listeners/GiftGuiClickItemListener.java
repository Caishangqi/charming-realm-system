...
public class GiftGuiClickItemListener implements Listener {
  ...
  public void onClick(InventoryClickEvent event) {
    ...
    if (event.getClickedInventory().getType() == InventoryType.PLAYER && (event.getCursor() == null || event.getCursor().getType() == Material.AIR)) {
      ...
      event.getWhoClicked().sendMessage(CharmRealm.pluginVariable.Lang_YML.getString("WantToPutItemIntoGiftChest"));
      return;
    } 
    ...
    if (i.hasItemMeta()) {
      ...
      if (meta.hasLore()) {
        ...
        for (int c = 0; c < lores.size(); c++) {
          if (((String)lores.get(c)).contains(CharmRealm.pluginVariable.Lang_YML.getString("GiftLoreAddPrefix"))) {
            lores.remove(c);
            meta.setLore(lores);
            has_lore = true;
            break;
